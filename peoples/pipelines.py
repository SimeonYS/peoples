...
class PeoplesPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `peoples`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM peoples WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO peoples (date, title, link, content)"
